...
namespace SetInOut
{
    public partial class Form1 : Form
    {
        ...
        private List<string> LoadRecentFilesFromRegistry()
        {
            // Open the registry key for the recent files list
            RegistryKey key = Registry.CurrentUser.OpenSubKey(@"Software\MyApp\RecentFiles");

            // Create a new list to hold the recent files
            List<string> recentFiles = new List<string>();

            // If the registry key exists, load the recent files from it
            if (key != null)
            {
                for (int i = 0; ; i++)
                {
                    ...
                    // If there is no more file path, stop the loop
                    if (filePath == null)
                    {
                        break;
                    }
                    // If the file path already exists in the recent files list, skip it
        ...
        public Form1()
        {
            ...
            this.dataDiCompilazione = "05/04/2023";
            ...
            recentFiles = LoadRecentFilesFromRegistry();
            LoadRecentFilesMenu(recentFiles);
        }
        [STAThread]
        ...
        private void AddRecentFile(string filePath)
        {
            ...
            if (recentFiles.Contains(filePath))
            {
                // If it does, remove it from its current position
                recentFiles.Remove(filePath);
            }
            recentFiles.Insert(0, filePath);

            // Remove duplicates from the recent files list
            recentFiles = recentFiles.Distinct().ToList();

            // Keep only the first 10 recent files in the list
            if (recentFiles.Count > 10)
            {
                recentFiles.RemoveRange(10, recentFiles.Count - 10);
            }

            // Save the updated recent files list to the registry
        ...
        private void LoadRecentFilesMenu(List<string> recentFiles)
        {
            // Clear the existing items in the ToolStripDropDownButton
            //FiletSDDB.DropDownItems.Clear();
            Debug.Print(recentFiles.ToString());
            // Loop over the recent files list and create a new ToolStripMenuItem for each file
            foreach (string filePath in recentFiles)
            {
                ...
                // Attach an event handler to open the file when the ToolStripMenuItem is clicked
                item.Click += (sender, e) =>
                {
                    // Get the full file path from the Tag property of the ToolStripMenuItem
                    string filePath1 = (string)((ToolStripMenuItem)sender).Tag;

                    // Open the file using your existing code to load the file
                  
                    this.richTextBox1.LoadFile(filePath1, RichTextBoxStreamType.PlainText);
                    this.ImpostaNomeFileNelTitolo(filePath1);
                };

                
                // Add the ToolStripMenuItem to the ToolStripDropDownButton
                FiletSDDB.DropDownItems.Add(item);
                FiletSDDB.Enabled = recentFiles.Count > 0;
            }
        }
        ...
        private void saveToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            if (flag)
            {
                ...
                AddRecentFile(saveFileDialog.FileName);
        ...
        private void FiletSDDB_Click(object sender, EventArgs e)
        {
        ...
        private void toolStripButton1_Click(object sender, EventArgs e)
        {
            string text = string.Concat(new string[]
            {
                ...
                "\n commands:", 
                "\n D*** = Timer in ms", 
                "\n Capital I*** OR O*** = input or Output",
                "\n ; = comment"
            });
