            ...
            for (int index1 = 0; flag && index1 < lines.Length; ++index1)
            {
                ...
                else
                {
                    ...
                    switch (enumerator.Current)
                    {
                        ...
                        case 'I': // Added case for if statement
                            ...
                            this.listBox1.Items.Add((object)r);
                            Form2.istrList[this.numIstr] = new Instruct('I', 0, 0, r);
                            ++this.numIstr;
                            Form2.istrList[this.numIstr] = new Instruct('T', 0, 0, strThen);
                            ++this.numIstr;
                            if (strIf == "1")
                            {
                                int numElse = r.IndexOf("ELSE", numThen + 4, r.Length - numThen - 4);
                                if (numElse < 0)
                                {
                                    this.listBox1.Items.Add((object)(r + " <- errore: manca l'ELSE"));
                                    flag = false;
                                    break;
                                }
                                string strElse = r.Substring(numThen + 4, numElse - numThen - 4).Trim();
                                if (strElse.Length == 0)
                                {
                                    this.listBox1.Items.Add((object)(r + " <- errore: manca l'istruzione ELSE"));
                                    flag = false;
                                    break;
                                }
                                Form2.istrList[this.numIstr] = new Instruct('E', 0, 0, strElse);
                                ++this.numIstr;
                                break;
                                        case 'E': // Added case for end if statement
                            if (r != "END IF")
                            {
                                this.listBox1.Items.Add((object)(r + " <- errore: la sintassi dell'istruzione END IF non è corretta"));
                                flag = false;
                                break;
                            }
                            this.listBox1.Items.Add((object)r);
